...
# xl.files <- list.files("~/Dropbox (The Francis Crick)/semi_oops_project_v2/V2/global_iCLIP/Naive/", "xl.bed", full.names = T)
...
```{r}
# naive_scatter <- plot_smoops_scatter(semi_naive.ashr, oops_naive.ashr, stage = "nPSCs", max_padj, min_lfc)
# epi_scatter <- plot_smoops_scatter(semi_epi.ashr, oops_epi.ashr, stage = "pPSCs", max_padj, min_lfc)
# diff_scatter <- plot_smoops_scatter(semi_diff.ashr, oops_diff.ashr, stage = "dPSCs", max_padj, min_lfc)
# 
...
# ggsave(paste0(figures.dir, "figure_1/smoops_lfc_correlation_same_axis.pdf"), corr.gg, dpi = 300, height = 16, width = 7)
```
...
```{r}
# master.df <- fread(paste0(base.results.dir, "Master Dataset/all_smOOPS_and_controls_V2.tsv"))
```
...
# # Plot
# both.txlength.long.gg <- ggplot(both.txlength.long.df %>% dplyr::filter(length >= 1), aes(x = group, y = length)) +
...
# ggsave(paste0(figures.dir, "figure_1/smoops_vs_nonsmoops_lengths.pdf"), both.txlength.long.gg, dpi = 300, width = 8, height = 8)
